...
public class FlagDAO {
    ...
    public void add(Tb_flag tb_flag) {
        ...
        db.execSQL("insert into tb_flag (id,name,time,flag) values (?,?,?,?)", new Object[] {
                tb_flag.getId(), tb_flag.getName(),Tb_flag.getTime(),tb_flag.getFlag() });// 执行添加便签信息操作
    }
    ...
    public void update(Tb_flag tb_flag) {
        db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
        db.execSQL("update tb_flag set flag = ? where id = ?", new Object[] {
    ...
    public Tb_flag find(int id) {
        ...
        Cursor cursor = db.rawQuery(
                "select id,name,time,flag from tb_flag where id = ?",
                new String[] { String.valueOf(id) });// 根据编号查找便签信息，并存储到Cursor类中
        if (cursor.moveToNext())// 遍历查找到的便签信息
        {
            // 将遍历到的便签信息存储到Tb_flag类中
            return new Tb_flag(cursor.getString(cursor.getColumnIndex("id")),
    ...
    public void detele(Integer... ids) {
        if (ids.length > 0)// 判断是否存在要删除的id
        {
            ...
            // 执行删除便签信息操作
            db.execSQL("delete from tb_flag where id in (" + sb + ")",
                    (Object[]) ids);
        }
    }
    ...
    public List<Tb_flag> getScrollData(int start, int count) {
        ...
        {
            // 将遍历到的便签信息添加到集合中
            lisTb_flags.add(new Tb_flag(cursor.getString(cursor.getColumnIndex("id")),
                    cursor.getString(cursor.getColumnIndex("name")),
                    cursor.getString(cursor.getColumnIndex("time")),
                    cursor.getString(cursor.getColumnIndex("flag"))));
        }
    ...
    public long getCount() {
        db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
        Cursor cursor = db.rawQuery("select count(id) from tb_flag", null);// 获取便签信息的记录数
    ...
    public int getMaxId() {
        db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
        Cursor cursor = db.rawQuery("select max(id) from tb_flag", null);// 获取便签信息表中的最大编号
